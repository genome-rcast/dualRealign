...
public class Align {
	...
	private static final int CORRECT_BASE_QUALITY_PENALTY = 20; // TODO: should
																// be a
																// parameter to
																// SRMA

	private static final List<String> saveTags = Arrays.asList("RG", "LB",
			"PU", "PG", "CS", "CQ");

	public static void align(Graph graph, SAMRecord rec, Node recNode,
			ReferenceSequence sequence, SAMProgramRecord programRecord,
			int offset, AlleleCoverageCutoffs alleleCoverageCutoffs,
			boolean correctBases, boolean useSequenceQualities,
			int MAXIMUM_TOTAL_COVERAGE, int MAX_HEAP_SIZE) throws Exception {
		...
		boolean strand = rec.getReadNegativeStrandFlag(); // false -> forward,
															// true -> reverse
		...
		if (space == SRMAUtil.Space.NTSPACE) {
			...
			// Some aligners include a quality value for the adapter. A quality
			// value
			// IMHO should not be given for an unobserved (assumed) peice of
			// data. Trim
			// the first quality in this case
			if (qualities.length() == 1 + read.length()) { // trim the first
															// quality
		...
		if (qualities.length() <= 0) {
			throw new Exception(
					"Error.  The current alignment has no qualities.");
		}
		if (readBases.length() != read.length()) {
			if (space == SRMAUtil.Space.COLORSPACE) {
				...
				throw new Exception(
						"Error.  Internal error: readBases.length() != read.length()");
			}
		}
		...
		// - save the soft clipped sequence for latter
		{
			...
			if (CigarOperator.S == e1.getOperator()) {
				...
				int l = e1.getLength();
				if (strand) { // reverse
					softClipStartBases = readBases.substring(readBases.length()
							- l);
					softClipStartQualities = qualities.substring(qualities
							.length() - l);
			...
			if (CigarOperator.S == e2.getOperator()) {
				...
				if (strand) { // reverse
					...
					softClipEndBases = readBases.substring(readBases.length()
							- l);
					softClipEndQualities = qualities.substring(qualities
							.length() - l);
					readBases = readBases.substring(0, readBases.length() - l);
					read = read.substring(0, read.length() - l);
					qualities = qualities.substring(0, qualities.length() - l);
		...
		comp = new AlignHeapNodeComparator(
				(strand) ? AlignHeap.HeapType.MAXHEAP
						: AlignHeap.HeapType.MINHEAP);

		// Bound by original alignment if possible
		bestAlignHeapNode = Align.boundWithOriginalAlignment(rec, graph,
				recNode, comp, strand, read, qualities, readBases, space,
				sequence, alleleCoverageCutoffs, useSequenceQualities,
				MAXIMUM_TOTAL_COVERAGE, MAX_HEAP_SIZE);

		/*
		 * System.err.println("readName="+rec.getReadName()); if(null !=
		 * bestAlignHeapNode) { System.err.println("\nFOUND BEST:" +
		 * rec.toString()); } else { System.err.println("\nNOT FOUND (BEST): " +
		 * rec.toString()); } Align.updateSAM(rec, programRecord,
		 * bestAlignHeapNode, space, read, qualities, softClipStartBases,
		 * softClipStartQualities, softClipEndBases, softClipEndQualities,
		 * strand, correctBases); return;
		 */

		heap = new AlignHeap((strand) ? AlignHeap.HeapType.MAXHEAP
				: AlignHeap.HeapType.MINHEAP);
		...
		if (strand) { // reverse
			...
			for (i = alignmentStart + offset; alignmentStart - offset <= i; i--) {
				...
				PriorityQueue<Node> startNodeQueue = graph
						.getPriorityQueue(position);
				if (0 != position && null != startNodeQueue) {
					Iterator<Node> startNodeQueueIter = startNodeQueue
							.iterator();
					while (startNodeQueueIter.hasNext()) {
						Node startNode = startNodeQueueIter.next();
						int f = passFilters(graph, startNode,
								alleleCoverageCutoffs, MAXIMUM_TOTAL_COVERAGE);
						if (0 == f) {
							heap.add(new AlignHeapNode(null, startNode,
									startNode.coverage, read.charAt(0),
									qualities.charAt(0), useSequenceQualities,
									space));
						} else if (f < 0) {
							return;
						}
			...
			for (i = alignmentStart - offset; i <= alignmentStart + offset; i++) {
				int position = graph
						.getPriorityQueueIndexAtPositionOrGreater(i);
				PriorityQueue<Node> startNodeQueue = graph
						.getPriorityQueue(position);
				if (0 != position && null != startNodeQueue) {
					Iterator<Node> startNodeQueueIter = startNodeQueue
		...
		while (null != curAlignHeapNode) {
			...
			// System.err.println("strand:" + strand + "\tsize:" + heap.size() +
			// "\talignmentStart:" + alignmentStart + "\toffset:" + offset +
			// "\treadOffset:" + curAlignHeapNode.readOffset);
			// System.err.print("size:" + heap.size() + ":" +
			// curAlignHeapNode.readOffset + ":" + curAlignHeapNode.score + ":"
			// + curAlignHeapNode.alleleCoverageSum + ":" +
			// curAlignHeapNode.startPosition + "\t");
			// curAlignHeapNode.node.print(System.err);
			// System.err.print("\rposition:" + curAlignHeapNode.node.position +
			// "\treadOffset:" + curAlignHeapNode.readOffset);

			// Remove all non-insertions with the same
			// contig/pos/read-offset/type/base and lower score
			...
			while (Node.INSERTION != curAlignHeapNode.node.type
				...
				if (curAlignHeapNode.score < nextAlignHeapNode.score
						|| (curAlignHeapNode.score == nextAlignHeapNode.score && curAlignHeapNode.alleleCoverageSum < nextAlignHeapNode.alleleCoverageSum)) {
					// Update current node
					curAlignHeapNode = heap.poll();
				} else {
					// Ignore next node
			...
			if (curAlignHeapNode.readOffset == read.length() - 1) {
				...
				// System.err.print(curAlignHeapNode.alleleCoverageSum + ":" +
				// curAlignHeapNode.score + ":");
				// System.err.print(curAlignHeapNode.startPosition + ":");
				// curAlignHeapNode.node.print(System.err);

				if (null == bestAlignHeapNode
						|| bestAlignHeapNode.score < curAlignHeapNode.score
						|| (bestAlignHeapNode.score == curAlignHeapNode.score && bestAlignHeapNode.alleleCoverageSum < curAlignHeapNode.alleleCoverageSum)) {
					bestAlignHeapNode = curAlignHeapNode;
				}
			} else if (null != bestAlignHeapNode
					&& curAlignHeapNode.score < bestAlignHeapNode.score) {
				// ignore, under the assumption that scores can only become more
				// negative.
			} else {
				if (strand) { // reverse
				...
				while (iter.hasNext()) {
					...
					int f = passFilters(graph, next.node, next.coverage,
							alleleCoverageCutoffs, MAXIMUM_TOTAL_COVERAGE);
					if (0 == f) {
						heap.add(new AlignHeapNode(
								curAlignHeapNode,
								next.node,
								next.coverage,
								read.charAt(curAlignHeapNode.readOffset + 1),
								qualities
										.charAt(curAlignHeapNode.readOffset + 1),
								useSequenceQualities, space));
					} else if (f < 0) {
						return;
					}
				}
		...
		// Recover alignment
		Align.updateSAM(rec, sequence, programRecord, bestAlignHeapNode, space,
				read, qualities, softClipStartBases, softClipStartQualities,
				softClipEndBases, softClipEndQualities, strand, correctBases);
	}
	...
	private static AlignHeapNode boundWithOriginalAlignment(
			...
			SRMAUtil.Space space, ReferenceSequence sequence,
			AlleleCoverageCutoffs alleleCoverageCutoffs,
			boolean useSequenceQualities, int MAXIMUM_TOTAL_COVERAGE,
			int MAX_HEAP_SIZE) throws Exception {
		AlignHeapNode curAlignHeapNode = null;
		AlignHeapNode nextAlignHeapNode = null;
		AlignHeapNode bestAlignHeapNode = null;
		ListIterator<NodeRecord> iter = null;
		...
		// Cannot bound
		if (0 != passFilters(graph, recNode, alleleCoverageCutoffs,
		...
		// Add start nodes
		heap.add(new AlignHeapNode(null, recNode, recNode.coverage, read
				.charAt(0), qualities.charAt(0), useSequenceQualities, space));
		...
		while (null != curAlignHeapNode) {
			...
			if (curAlignHeapNode.readOffset == readBases.length() - 1) { // found,
																			// keep
																			// beset
				if (null == bestAlignHeapNode
						|| bestAlignHeapNode.score < curAlignHeapNode.score
	...
	private static void updateSAM(SAMRecord rec, ReferenceSequence sequence,
			...
			SRMAUtil.Space space, String read, String qualities,
			String softClipStartBases, String softClipStartQualities,
			String softClipEndBases, String softClipEndQualities,
			boolean strand, boolean correctBases) throws Exception {
		AlignHeapNode curAlignHeapNode = null;
		...
		while (null != curAlignHeapNode) {
			...
			if (null != prevAlignHeapNode
					...
					&& 1 < Math.abs(curAlignHeapNode.node.position
							- prevAlignHeapNode.node.position)) {
			...
			if (prevCigarOperator != curCigarOperator) {
				...
				if (null != prevCigarOperator) {
					if (strand) { // reverse
						// append
						cigarElements.add(new CigarElement(
								prevCigarOperatorLength, prevCigarOperator));
					} else {
						// prepend
						cigarElements.add(0, new CigarElement(
								prevCigarOperatorLength, prevCigarOperator));
					}
				}
				...
				if (curCigarOperator == CigarOperator.DELETION) {
					// length of deletion
					prevCigarOperatorLength = Math
							.abs(curAlignHeapNode.node.position
									- prevAlignHeapNode.node.position) - 1;
		...
		if (0 < prevCigarOperatorLength) {
			if (null == prevCigarOperator
					|| CigarOperator.DELETION == prevCigarOperator) {
				throw new Exception(
						"Ended with a null cigar operator or a deletion cigar operator");
			}
			if (strand) { // reverse
				// append
				cigarElements.add(new CigarElement(prevCigarOperatorLength,
						prevCigarOperator));
			} else {
				// prepend
				cigarElements.add(0, new CigarElement(prevCigarOperatorLength,
						prevCigarOperator));
			}
		}

		if (space == SRMAUtil.Space.COLORSPACE) { // color space, read bases
													// already inferred
			...
			if (strand) { // reverse
				for (i = 0; i < read.length(); i++) {
					char nextBase = SRMAUtil.colorSpaceNextBase(prevBase,
							read.charAt(i));
					if (nextBase == SRMAUtil
							.getCompliment((char) readBases[read.length() - i
									- 1])) {
					...
					if (0 < i) {
						// qualities are assumed to be always in the same
						// direction as the color errors
						baseQualities[read.length() - i] = getColorQuality(
								colorErrors[i - 1], colorErrors[i],
								(byte) (qualities.charAt(i - 1) - 33),
								(byte) (qualities.charAt(i) - 33));
					}
					prevBase = SRMAUtil.getCompliment((char) readBases[read
							.length() - i - 1]);
				}
				// last color
				...
				for (i = 0; i < read.length(); i++) {
					...
					if (0 < i) {
						baseQualities[i - 1] = getColorQuality(
								colorErrors[i - 1], colorErrors[i],
								(byte) (qualities.charAt(i - 1) - 33),
								(byte) (qualities.charAt(i) - 33));
					}
				...
				// last color
				baseQualities[read.length() - 1] = (byte) (qualities
						.charAt(read.length() - 1) - 33);
			}
		} else if (correctBases) { // bases were corrected
			if (strand) {
				for (i = 0; i < read.length(); i++) {
					if (readBases[i] == (byte) read.charAt(read.length() - i
							- 1)) {
						baseQualities[i] = (byte) (qualities.charAt(read
								.length() - i - 1) - 33);
					} else {
						// TODO: how much to down-weight ?
						baseQualities[i] = (byte) (SRMAUtil.QUAL2CHAR(SRMAUtil
								.CHAR2QUAL(qualities.charAt(read.length() - i
										- 1))
								- CORRECT_BASE_QUALITY_PENALTY) - 33);
			...
			readBases = new byte[read.length()];
			baseQualities = new byte[qualities.length()]; // qualities.length()
															// == read.length()
			if (strand) { // reverse
				for (i = 0; i < read.length(); i++) {
					readBases[i] = (byte) read.charAt(read.length() - i - 1);
					baseQualities[i] = (byte) (qualities.charAt(read.length()
							- i - 1) - 33);
				}
			} else {
				for (i = 0; i < read.length(); i++) {
		...
		if (null != softClipStartBases) { // prepend
			cigarElements.add(0, new CigarElement(softClipStartBases.length(),
					CigarOperator.S));

			byte tmpBases[] = new byte[readBases.length
					+ softClipStartBases.length()];
			System.arraycopy(readBases, 0, tmpBases,
					softClipStartBases.length(), readBases.length);
			readBases = tmpBases;
			for (i = 0; i < softClipStartBases.length(); i++) {
				readBases[i] = (byte) softClipStartBases.charAt(i);
			}

			byte tmpQualities[] = new byte[baseQualities.length
					+ softClipStartQualities.length()];
			System.arraycopy(baseQualities, 0, tmpQualities,
					softClipStartQualities.length(), baseQualities.length);
			baseQualities = tmpQualities;
			for (i = 0; i < softClipStartQualities.length(); i++) {
				baseQualities[i] = (byte) (softClipStartQualities.charAt(i) - 33);
			}
		}
		if (null != softClipEndBases) { // append
			cigarElements.add(new CigarElement(softClipEndBases.length(),
					CigarOperator.S));

			byte tmpBases[] = new byte[readBases.length
					+ softClipEndBases.length()];
			System.arraycopy(readBases, 0, tmpBases, 0, readBases.length);
			for (i = 0; i < softClipEndBases.length(); i++) {
				tmpBases[i + readBases.length] = (byte) softClipEndBases
						.charAt(i);
			}
			readBases = tmpBases;

			byte tmpQualities[] = new byte[baseQualities.length
					+ softClipEndQualities.length()];
			System.arraycopy(baseQualities, 0, tmpQualities, 0,
					baseQualities.length);
			for (i = 0; i < softClipEndQualities.length(); i++) {
				tmpQualities[i + baseQualities.length] = (byte) (softClipEndQualities
						.charAt(i) - 33);
			}
			baseQualities = tmpQualities;
		}
	...
	private static int getmatch(SAMRecord rec) {
	...
	/*
	 * -1 if the alignment process should be aborted 0 if the alignment should
	 * continue 1 if the alignment should not be considered any further
	 */
	private static int passFilters(Graph graph, Node node, int toNodeCoverage,
			AlleleCoverageCutoffs alleleCoverageCutoffs,
	...
	private static int passFilters(Graph graph, Node node,
			AlleleCoverageCutoffs alleleCoverageCutoffs,
			int MAXIMUM_TOTAL_COVERAGE) {
	...
	private static void clearAttributes(SAMRecord rec,
			List<String> optFieldTags, List<Object> optFieldValues) {
	...
	private static void resetAttributes(SAMRecord rec,
			List<String> optFieldTags, List<Object> optFieldValues) {
