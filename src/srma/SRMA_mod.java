 ...
 * LICENSE to be determined
 This class was modfied by hiroki ueda
 from  a original version of SRMA
 */
package srma;
...
//	@Option(shortName = StandardOptionDefinitions.INPUT_SHORT_NAME, doc = "The input SAM or BAM file.")
	...
	private final static int SRMA_OFFSET_PROCESS = 100; // the number of extra
														// offset bases until we
														// should process the
														// graph
	...
	private SAMRecordIO_mod io = null;
	...
	private SAMFileHeader fileHeader = null;

//	public static void main(final String[] args) {
//		new SRMA_mod().instanceMain(args);
//	}
//
...
//	}
	...
	/*
	 * Current assumptions: - can fit entire partial order graph in memory
	 */
	public List<SAMRecord> doWorkSRMA(SAMFileHeader fileHeader,
			ReferenceSequence rsf, ReferenceSequenceFile  referenceSequenceFile , String chr, int start,
 int end, List<SAMRecord> in, int numOfThread) throws Exception {
		int ctr = 0;
		AlignRecord rec = null;
		this.referenceSequence = rsf;
		this.fileHeader = fileHeader;


			// mod
			this.referenceSequenceFile = referenceSequenceFile;
			this.referenceDictionary = this.referenceSequenceFile
					.getSequenceDictionary();

			this.inputRanges = new Ranges(this.referenceDictionary);
			this.outputRanges = new Ranges(this.referenceDictionary);
			this.io = new SAMRecordIO_mod(in, this.referenceDictionary);
			...
			this.NUM_THREADS = numOfThread;
			
			this.outputRange = this.outputRangesIterator.next();

			// init
			this.alleleCoverageCutoffs = new AlleleCoverageCutoffs(
					MINIMUM_ALLELE_COVERAGE, MINIMUM_ALLELE_PROBABILITY,
					QUIET_STDERR);
			this.toOutputQueue = new PriorityQueue<AlignRecord>(40,
					new AlignRecordComparator());
...
//						|| inputRange.referenceIndex != this.referenceSequence
//								.getContigIndex()) {
//					this.referenceSequence = this.referenceSequenceFile
//							.getSequence(this.referenceDictionary.getSequence(
//									inputRange.referenceIndex)
//									.getSequenceName());
//					if (null == this.referenceSequence) {
//						throw new Exception(
//								"Premature EOF in the reference sequence");
//					} else if (this.referenceSequence.getContigIndex() != inputRange.referenceIndex) {
//						throw new Exception(
//								"Could not find the reference sequence");
//					}
//				}

				if (null != rec) {
					// do an initial prune
					this.graph.prune(rec.record.getReferenceIndex(),
							rec.record.getAlignmentStart(), 0,
							this.GRAPH_PRUNING);
				}

				while (null != rec) {
					if (rec.record.getReadUnmappedFlag()
							|| rec.record.getNotPrimaryAlignmentFlag()) {
						// TODO
						...
						if (rec.record.getReferenceIndex() < prevReferenceIndex
								|| (rec.record.getReferenceIndex() == prevReferenceIndex && rec.record
										.getAlignmentStart() < prevAlignmentStart)) {
							throw new Exception(
									"SAM/BAM file is not co-ordinate sorted.");
						}

						// check if we should flush the previous records
						if (prevReferenceIndex != curReferenceIndex
								|| prevAlignmentStart + this.OFFSET
										+ this.SRMA_OFFSET_PROCESS < curAlignmentStart) {
							// process graph
			...
			if (!QUIET_STDERR) {
				...
				// Memory
				double totalMemory = (double) Runtime.getRuntime()
						.totalMemory();
				double totalMemoryLog2 = Math.log(totalMemory) / Math.log(2.0);
				if (totalMemoryLog2 < 10) {
					System.err.println("Total memory usage: "
							+ (int) totalMemory + "B");
				} else if (totalMemoryLog2 < 20) {
					System.err
							.println("Total memory usage: "
									+ (Math.round(100 * totalMemory
											/ Math.pow(2, 10)) / 100) + "KB");
				} else {
					System.err
							.println("Total memory usage: "
									+ (Math.round(100 * totalMemory
											/ Math.pow(2, 20)) / 100) + "MB");
				}
				// Run time
				...
				seconds -= minutes * 60;
				System.err.println("Total execution time: " + hours + "h : "
						+ minutes + "m : " + seconds + "s");
			}
	...
	private AlignRecord getNextAlignRecord() throws Exception {
		if (this.io.hasNextAlignRecord()) {
			// TODO if we have only a subsequence of a contig, we need to check
			// bounds
	...
	private void outputProgress(SAMRecord rec, int ctr) {
		if (QUIET_STDERR) {
			...
			int i;
			String outputString = new String("Records processsed: " + ctr
					+ " (last " + rec.getReferenceName() + ":"
	...
	private void processToAddToGraphList() throws Exception {
		// Process alignments
		if (0 < this.toAddToGraphList.size()) {
			...
			if (0 == this.toAlignList.size()
					&& this.graph.contig != this.toAddToGraphList.getFirst().record
							.getReferenceIndex() + 1) {
				// Move to a new contig
				this.graph.prune(this.toAddToGraphList.getFirst().record
						.getReferenceIndex(),
						this.toAddToGraphList.getFirst().record
								.getAlignmentStart(), 0, this.GRAPH_PRUNING);
			}

			// Get the records for the threads
			toAddToGraphThreadLists = toAddToGraphList.getThreadLists(
					this.NUM_THREADS, this.graph.contig);
			...
			for (i = 0; i < this.NUM_THREADS; i++) {
				...
				threads.add(new GraphThread(i, toAddToGraphThreadLists.get(i)
						.listIterator(), toAlignThreadLists.get(i)));
			}
...
//				threads.get(i).join();
//			}
			threads.get(0).run();// Change by Ueda, no new thread, since whole process is
			//run by parent thread
	...
	private int processToAlignList(int ctr, boolean flush) throws Exception {
		...
		if (0 < this.toAlignList.size()) {
			...
			if (flush
					|| this.toAlignList.getFirst().record.getAlignmentEnd()
							+ OFFSET < this.toAlignList.getLast().record
							.getAlignmentStart()) {
				int i, size;
				LinkedList<Thread> threads = null;
				LinkedList<LinkedList<AlignRecord>> toAlignThreadLists = null;
				 ...
				 * ) { System.err.println(i + "\t" + this.graph.getCoverage(i));
				 * }
				 */

				// Get thread data
				if (flush) {
					toAlignThreadLists = this.toAlignList.getThreadLists(
							this.NUM_THREADS, this.graph.contig);
				} else {
					toAlignThreadLists = this.toAlignList
							.getAlignRecordThreadLists(
									this.NUM_THREADS,
									this.graph.contig,
									this.toAlignList.getLast().record
											.getAlignmentStart() - this.OFFSET);
				}

				SAMProgramRecord prc = new SAMProgramRecord("srma");
				// Create threads
				threads = new LinkedList<Thread>();
				for (i = 0; i < this.NUM_THREADS; i++) {
					threads.add(new AlignThread(i,prc, toAlignThreadLists
							.get(i).listIterator()));
				}
...
//				}
				...
				if (null != lastSAMRecord) {
					...
					if (0 < toAlignList.size()) {
						this.graph.prune(toAlignList.getFirst().record
								.getReferenceIndex(),
								toAlignList.getFirst().record
										.getAlignmentStart(), this.OFFSET,
								this.GRAPH_PRUNING);
					} else {
						this.graph.prune(lastSAMRecord.getReferenceIndex(),
								lastSAMRecord.getAlignmentStart(), this.OFFSET,
								this.GRAPH_PRUNING);
					}
				}
			}
		...
		while (0 < this.toOutputQueue.size()) {
			...
			// alignment could have moved (+OFFSET), with another moving
			// (-OFFSET)
			if (rec.record.getReferenceIndex() + 1 != graph.contig || // different
																		// contig
					rec.record.getAlignmentStart() + 2 * OFFSET + 50 < graph.position_start) { // other
																								// alignments
																								// will
																								// not
																								// be
	...
	private boolean recordAlignmentStartContained(SAMRecord rec) {
		...
		while (this.outputRange.referenceIndex < recReferenceIndex
				|| (this.outputRange.referenceIndex == recReferenceIndex && this.outputRange.endPosition < recAlignmentStart)) { // find
																																	// a
																																	// new
																																	// range
		...
		if (recReferenceIndex < this.outputRange.referenceIndex
				|| (recReferenceIndex == this.outputRange.referenceIndex && recAlignmentStart < this.outputRange.startPosition)) { // before
																																	// range
			// not within range
			return false;
		} else {
	...
	private void InsertSizeRangeSet() throws Exception {
		String low, high;
		int colon;
		...
		InsertSizeRangeLow = Integer.parseInt(INSERT_SIZE_RANGE.substring(0,
				colon));
		InsertSizeRangeHigh = Integer.parseInt(INSERT_SIZE_RANGE
				.substring(colon + 1));
	...
	private class AlignThread extends Thread {
		...
		public AlignThread(int threadID, SAMProgramRecord programRecord,
			...
			try {
				...
				while (iter.hasNext()) {
					...
					Align.align(graph, curSAMRecord, curNode,
							referenceSequence, this.programRecord, OFFSET,
							alleleCoverageCutoffs, CORRECT_BASES,
							USE_SEQUENCE_QUALITIES, MAXIMUM_TOTAL_COVERAGE,
							MAX_HEAP_SIZE);
				}
				...
				e.printStackTrace();
				System.err
						.println("Please report bugs to srma-help@lists.sourceforge.net");
				//System.exit(1);
